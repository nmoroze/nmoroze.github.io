...
Notary resets the state of its CPU in two steps between running each agent. First, the chip is hardware reset, which clears some data. Next, boot code runs, clearing the rest of the data.
{: .caption }
...
There are many open-source RISC-V reference hardware designs freely available on the internet. We ended up picking [OpenTitan](opentitan.org) for the following reasons:
...
The first challenge is that OpenTitan contains storage components that cannot be cleared by boot code, meaning it cannot satisfy Notary’s deterministic start property. To use such a system with minimal modifications to the hardware, we had to define a new property that provides the same isolation guarantees.
...
Proving this with code may seem infeasible since we have to show that the outputs would be the same even for an infinite sequence of circuit steps. However, we can turn this into a finite problem by using the mathematical technique of _induction_. The idea is that we actually only need to prove two things:
*   The base case, that this property holds for cycle 0 (the initial state of the circuit)
*   The inductive step, that assuming our property holds for cycle _n_, it must also hold for cycle _n+1_

Proving these two claims implies that the property holds for all cycles _n_, without having to exhaustively show that this is the case. 
...
To prove the base case, we construct the two circuit instances as described above. We then verify that the outputs of each state are equal, and that they are _related_. We define in the verification code what it means for two states to be related -- it depends on the specific circuit itself, but only certain definitions will allow the proof to go through. 
...
The most interesting output determinism violation was the SPI RX leak. This issue allows the CPU to read seven bits of uninitialized state left in the SPI peripheral prior to reset by toggling a configuration register at the right time. The SPI communication protocol receives data one bit at a time, and this configuration register controls the order in which bits are recombined to form full bytes of data. The register that stores these recombined bits in the hardware is left uninitialized on reset, meaning it initially contains whatever byte was last received via SPI. By toggling the configuration register in between every new bit that’s received, this register does not get completely filled with new data before the hardware counts that a full 8 bits of data have been received and sends the data off to the CPU. The following GIF illustrates how this works. 

![RX leak animation]({{ page.imgdir }}/rx-leak.gif){: style="box-shadow:none;"}
{: style="text-align:center;"}

Animation of the SPI RX leak. The red numbers indicate each bit, labelled by index, that are left over in the `rx_data` register from the previous execution. The green numbers are the indices of the new bits shifted in. Note that once the counter hits 8 bits received, `rx_data` will still contain the uninitialized bits 0 - 6.
{: .caption}


### Verification stats

Overall, it takes about 3.5 hours to run Kronos's verification code. The code consists of about 1,000 lines (not counting various libraries and common code). Proving output determinism for the core clock domain takes more code and accounts for almost all of the runtime since it involves simulating the execution of boot code over thousands of cycles. By comparison, proving output determinism for a peripheral clock domain simply involves proving the base case and inductive step. 

| Clock domain      | Lines of Code | Runtime (seconds) |
|-------------------|---------------|-------------------|
| Core clock domain | 464           | 12,526.5          |
| USB               | 303           | 41.1              |
| SPI-in            | 130           | 18.4              |
| SPI-out           | 128           | 17.8              |
...
Through Kronos, we showed MicroTitan provides the necessary security guarantees to be used in a secure Notary prototype. To do so, we defined a new security property called _output determinism_ and an approach called _modular output determinism_ to formally verify this property for a circuit with multiple clock domains. 
...
Overall, Kronos is an interesting case study in formally verifying a security property at the hardware level, which is important for making strong security guarantees about a system without relying on assumptions about the system’s underlying hardware. 
